...
def test_configuration_integration():
    ...
    try:
        ...
        default_path = config.get('download', 'default_output_path')
        assert default_path == 'source-files', f"Expected 'source-files', got '{default_path}'"
        
        # Test custom configuration
        config.set('download', 'default_output_path', 'source-files/custom')
        custom_path = config.get('download', 'default_output_path')
        assert custom_path == 'source-files/custom', f"Expected 'source-files/custom', got '{custom_path}'"
        ...
        assert env_path == 'source-files/env', f"Expected 'source-files/env', got '{env_path}'"
...
def main():
    ...
    if passed == total:
        ...
        logger.info("📁 The application now stores all files in the 'source-files' directory")
        logger.info("🔧 You can customize subdirectories using configuration or environment variables")
        return 0
    else:
        logger.error("💥 Some source-files structure tests failed. Please check the implementation.")
        return 1
