...
def main():
    ...
    try:
        ...
        if result_standard["success"]:
            logger.success(
                f"Standard transcription completed: {result_standard['transcription_file']}"
            )
        else:
        ...
        logger.info(
            "=== EXAMPLE 2: Persona Transcription with Speaker Identification ==="
        )
        ...
        if result_persona["success"]:
            logger.success(
                f"Persona transcription completed: {result_persona['transcription_file']}"
            )

            # Compare the outputs
            logger.info("=== COMPARISON ===")
            logger.info(
                f"Standard transcription: {result_standard['transcription_file']}"
            )
            logger.info(
                f"Persona transcription: {result_persona['transcription_file']}"
            )

            # Show preview of persona transcription
            if os.path.exists(result_persona["transcription_file"]):
                with open(
                    result_persona["transcription_file"], "r", encoding="utf-8"
                ) as f:
                    lines = f.read().split("\n")[:15]  # First 15 lines
