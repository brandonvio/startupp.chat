...
async def test_bluesky_post_generation():
    ...
    try:
        ...
        logger.info(
            f"Initializing OllamaAnalysisService with {ollama_url} using model {ollama_model}..."
        )
        ...
        post_content = await analysis_service.generate_bluesky_post(
            video_id=video_id, analysis_content=analysis_content
        )
        ...
        if len(post_content) <= 290 and hashtag_count >= 2:
            ...
            logger.warning("❌ Post validation issues:")
