...
async def test_ai_post_generation():
    ...
    **Technical Insights:**
    ...
    try:
        ...
        if hashtag_count >= 2:
            logger.success(f"✅ Post contains {hashtag_count} hashtags (meets requirement)")
        else:
            logger.warning(f"⚠️  Post contains only {hashtag_count} hashtags (needs at least 2)")
            
        # Check character count
        if len(post_content) <= 250:
            logger.success(f"✅ Post is within 250 character limit ({len(post_content)} chars)")
        else:
            logger.warning(f"⚠️  Post exceeds 250 character limit ({len(post_content)} chars)")
        
        # Overall assessment
        has_urls = "youtube.com" in post_content or "youtu.be" in post_content
        meets_criteria = (not has_urls) and (hashtag_count >= 2) and (len(post_content) <= 250)
        
        if meets_criteria:
            logger.success("🎉 POST MEETS ALL CRITERIA!")
        else:
            logger.warning("⚠️  Post does not meet all criteria, but iterative system should have improved it.")
